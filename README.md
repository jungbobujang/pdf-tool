...
| 도구 | 할 수 있는 일 |
|---|---|
| 편집 · 합치기 | PDF 여러 개를 한 화면에 펼쳐 놓고 끌어서 순서 바꾸기(마우스·터치). **여러 쪽 선택**(Ctrl/Shift+클릭, 체크 상자, 빈 곳에서 네모 끌기, 휴대폰은 길게 누르기) 후 한꺼번에 끌어 옮기기 · 맨 앞/맨 뒤/N쪽 다음으로 · 90° 회전 · 삭제 예정 · 선택한 쪽만 저장. 되돌리기/다시 하기 50단계. 쪽마다 ↺ ↻ 회전 · 다른 쪽으로 교체 · 삭제 예정 표시. 아래 저장 막대에서 범위만 저장(`1-2, 4-7`), 한 쪽씩 나눠 zip 저장, 홀수 · 짝수 · 역순, 합쳐서 저장. 잠긴 PDF는 노란 안내줄에서 비밀번호를 넣으면 함께 편집 |
...
```bash
npm run verify      # 합치기 · 범위 · 회전 · 교체 · 암호 · 쪽번호 · 여러 쪽 순서 조작 로직 검증 (node)
...
public/pdf-core.js   PDF 핵심 로직 + 쪽 순서 순수 함수(moveGroup · moveToFront · moveToEnd · moveAfter · rotate) (브라우저와 검증 스크립트가 함께 씀)
public/app.js        화면 동작
